...
import { GetAnimePlayerData, GetAnimeStreamingData } from '@/graphql/queries/GetAnime.gql'
...
const AnimePlayerPage = () => {
    ...
    const [animeData, setAnimeData] = useState<any>({ episodes: [{ id: "", title: "", description: "", image: "", number: "" }], zoroEpisodes: [{ zoroId: "" }] })
                        ...
                        <div className="flex h-full items-center justify-center p-6">
                            <ResizablePanel defaultSize={20} className='max-w-[500px] h-full'>
                                ...
                                <ScrollArea className="h-full w-full rounded-md border p-4 pb-12">
                                    {animeData?.episodes && animeData?.episodes.length > 0 && animeData?.episodes[0].id && <div className="flex flex-col gap-3">
                                        <div className="flex flex-col gap-3">
                                            {animeData?.episodes?.map((episode: { id: string; title: any; episode: string, number: string }, index: number) => (
                                                <Button key={index} variant={episodeNumber === (index as number) + 1 ? "default" : "secondary"} onClick={() => { setEpisodeNumber((index as number) + 1); setZoroId(animeData?.zoroEpisodes[index]?.id); setAnilistId(episode.id); }} title={`EP ${episode.number} ` + episode.title || animeData?.zoroEpisodes[index]?.title || `Episode ${episode.number}`} className="w-full whitespace-nowrap overflow-hidden text-ellipsis text-start justify-start">{`EP ${episode.number}: ` + episode.title || animeData?.zoroEpisodes[index]?.title || `Episode ${episode.number}`}</Button>
                                            ))}
                                        </div>
                                    </div>}
                    ...
                    <ResizablePanel className='h-[100dvh] w-full min-w-0 sm:p-0 pb-32' defaultSize={60}>
                        <div className="w-full h-full">
                            <ScrollArea className='w-full h-full'>
                                {!isAnimePlayer && streamingData && streamingData.length >= 8 && <iframe className="w-full overflow-y-hidden aspect-[1.78/1] outline-none" src={`${streamingData[8].headers.Referer || "https://imvt.vercel.app"}`} sandbox='allow-same-origin allow-scripts' referrerPolicy="strict-origin" allow="autoplay; fullscreen; geolocation; display-capture; picture-in-picture" allowFullScreen></iframe>}
                                ...
                                {(!loading && isAnimePlayer && !streamingLoading) && streamingData[serverNumber - 1]?.sources?.length > 0 && <MediaPlayer
                                    media={{
                                        urls: streamingData[serverNumber - 1].sources,
                                        subtitles: streamingData[serverNumber - 1].subtitles ? streamingData[serverNumber - 1].subtitles : [],
                                        thumbnail: animeData?.episodes ? animeData?.episodes[episodeNumber - 1]?.image : `data:image/${shimmerBlurDataUrl(1080, 720)}`,
                                ...
                                />}
                                {(!loading && isAnimePlayer && !streamingLoading) && (streamingData[serverNumber - 1]?.sources?.length === 0 || !streamingData[serverNumber - 1]?.sources || !streamingData) &&
                                ...
                                {(loading || streamingLoading || !animeData) && isAnimePlayer &&
                                    <div className='w-[99%] min-h-[200px] sm:min-h-[350px] max-h-[100dvh] aspect-[4/3] sm:aspect-[1.78/1] flex items-center justify-center'>
                                        <Image
                                ...
                                <div className="flex flex-col gap-5 items-start justify-center p-10 pt-10">
                                    <div className='flex sm:flex-row flex-col items-start justify-center gap-3'>
                                        ...
                                        <div className='flex flex-wrap items-center justify-start gap-3'>
                                            <Button onClick={(e) => { setServerNumber(9); setIsAnimePlayer(true) }} variant={isAnimePlayer && serverNumber === 9 ? "default" : "secondary"} className="w-28">Server 1</Button>
                                            <Button onClick={(e) => { setServerNumber(5); setIsAnimePlayer(true) }} variant={isAnimePlayer && serverNumber === 5 ? "default" : "secondary"} className="w-28">Server 2</Button>
                                            <Button onClick={(e) => { setServerNumber(1); setIsAnimePlayer(true) }} variant={isAnimePlayer && serverNumber === 1 ? "default" : "secondary"} className="w-28">Server 3</Button>
                                            {/* Not Working */}
                                            <Button onClick={(e) => { setServerNumber(3); setIsAnimePlayer(true) }} variant={isAnimePlayer && serverNumber === 3 ? "default" : "secondary"} className="w-28">Server 4</Button>
                                            <Button onClick={(e) => { setServerNumber(7); setIsAnimePlayer(true) }} variant={isAnimePlayer && serverNumber === 7 ? "default" : "secondary"} className="w-28">Server 5</Button>
                                        </div>
                                    </div>
                                    <div className='flex sm:flex-row flex-col items-start justify-center gap-3'>
                                        <span className='shrink-0 py-1'>
                                        ...
                                        <div className='flex flex-wrap items-center justify-start gap-3'>
                                            <Button onClick={(e) => { setServerNumber(6); setIsAnimePlayer(true) }} variant={isAnimePlayer && serverNumber === 6 ? "default" : "secondary"} className="w-28">Server 1</Button>
                                            <Button onClick={(e) => { setServerNumber(2); setIsAnimePlayer(true) }} variant={isAnimePlayer && serverNumber === 2 ? "default" : "secondary"} className="w-28">Server 2</Button>
                                            {/* Not Working */}
                                    ...
                                    <div className='flex sm:flex-row flex-col items-start justify-center gap-3'>
                                        ...
                                        <div className='flex flex-wrap items-center justify-start gap-3'>
                                            <Button onClick={(e) => { setIsAnimePlayer(false) }} variant={!isAnimePlayer ? "default" : "secondary"} className="w-28">Iframe 1</Button>
                                        </div>
                                    </div>
                                </div>
                                        ...
                                        <CardHeader className='flex flex-col gap-y-4'>
                                            ...
                                            <div className="w-full">
                                                <Image className='w-full max-h-[50dvh] object-contain object-left'
                                                    src={animeData?.episodes[episodeNumber - 1]?.image || `data:image/${shimmerBlurDataUrl(1080, 720)}`}
                                            ...
                                            <CardTitle>{animeData?.episodes[episodeNumber - 1]?.title}</CardTitle>
                                            {/* <CardTitle>{animeData?.zoroEpisodes[episodeNumber - 1]?.title}</CardTitle> */}
                                            <CardDescription>
                                                {animeData?.episodes[episodeNumber - 1]?.description || animeData?.description}
                                            </CardDescription>
                                        </CardHeader>
                                        <CardFooter>
                                            {animeData?.episodes[episodeNumber - 1]?.createdAt && new Date(animeData?.episodes[episodeNumber - 1]?.createdAt).toLocaleDateString("en-US", { month: 'short', day: 'numeric', year: 'numeric' })}
