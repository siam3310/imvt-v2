...
export default function AnimeDetailsTabs({ animeData }: { animeData: any }) {
  ...
  return (
    <Tabs defaultValue='more-details' className='w-[95%] sm:w-[90%] dark'>
          ...
          <CardContent className='w-full md:w-1/3 flex flex-col justify-center items-start gap-y-3'>
            {animeData?.startDate && (
              <CardTitle className='text-[1.5rem]'>
                Start Date{' '}
                <CardDescription className='text-[1.2rem] font-normal'>
                  {monthNames[animeData?.startDate?.month - 1]}{' '}
                  {animeData?.startDate?.day}, {animeData?.startDate?.year}
                </CardDescription>
              </CardTitle>
            )}
            {animeData?.endDate && (
              <CardTitle className='text-[1.5rem]'>
                End Date{' '}
                <CardDescription className='text-[1.2rem] font-normal'>
                  {monthNames[animeData?.endDate?.month - 1]}{' '}
                  {animeData?.endDate?.day}, {animeData?.endDate?.year}
                </CardDescription>
              </CardTitle>
            )}
            {animeData?.studios && (
            ...
            <CardDescription className='text-[1.2rem]'>
              Licensed: {animeData?.isLicensed ? 'True' : 'False'}
            </CardDescription>
            <CardDescription className='text-[1.2rem]'>
              Country of Origin: {animeData?.countryOfOrigin}
            </CardDescription>
            {animeData?.synonyms && (
              <CardTitle className='text-[1.5rem]'>Other Names</CardTitle>
            )}
            {animeData?.synonyms.map((synonym: string, index: number) => (
              <CardDescription className='text-[1.2rem]' key={index}>
                {synonym}
              </CardDescription>
            ))}
          </CardContent>
      ...
      {animeData?.characters && (
        <TabsContent value='characters' className='p-5'>
          <div className='flex justify-center items-start flex-wrap w-full text-center'>
            {animeData?.characters.map(
              (character: any, index: React.Key | number) => (
                <div
                  key={index}
                  className='border-2 border-slate-800/50 border-b-0 h-max hover:bg-slate-800/50 transition-all duration-300 ease-in-out rounded-t-full flex flex-col items-center gap-x-5 shrink-0 w-full sm:w-[50%] lg:w-[33%]'
                >
                  <Image
                  ...
                  <div className='flex flex-col justify-center items-center text-white gap-y-1'>
                    ...
                    <div className='flex flex-wrap max-h-[300px] py-3 overflow-y-scroll justify-evenly items-center gap-y-5'>
                      {character?.voiceActors.map(
                        (actor: any, index: React.Key | number) => (
                          <div
                            key={index}
                            className='flex-col flex items-center gap-x-5 w-[33%]'
                          >
                            <Image
                              className='rounded-full flex justify-center items-center w-24 h-24 aspect-square object-cover'
                              src={`${actor.image}`}
