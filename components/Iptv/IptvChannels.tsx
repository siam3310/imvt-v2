...
const IptvChannels = ({
  ...
  return (
    <div className='w-full max-h-[100dvh] flex flex-col gap-y-3'>
      {query && (
        <h1 className='text-white text-start font-semibold text-[1rem] flex gap-x-3'>
          {iptvData?.totalResults}{' '}
          {iptvData?.totalResults > 1 ? 'results' : 'result'} found for {query}
          <XCircle className='cursor-pointer' onClick={(e) => setQuery('')} />
        </h1>
      )}
      <div className='h-full pb-[100px] sm:pb-[30px]'>
            ...
            <div className='flex flex-wrap w-full justify-start items-center'>
              {iptvData?.results?.map((iptv, index) => (
                <div
                  ...
                  onClick={() => setIptvPlayerData(iptv)}
                  className={`relative min-w-0 shrink-0 grow-0 basis-1/2 sm:basis-1/4 lg:basis-1/6 2xl:basis-1/8 h-fit p-2`}
                >
                  <div className='group clickable'>
                    ...
                    <div className='w-full h-full'>
                      ...
                      <h2
                        className='text-gray-900 bg-white border  px-3 text-center whitespace-nowrap overflow-hidden text-ellipsis font-semibold'
                        title={iptv?.inf?.title}
                      >
                        {iptv?.inf?.title}
                      </h2>
                      <h2
                        onClick={(e) => setQuery(iptv?.inf?.groupTitle)}
                        className='bg-white rounded-b-md border px-3 text-center whitespace-nowrap overflow-hidden text-ellipsis font-semibold text-blue-600 cursor-pointer'
                        title={iptv?.inf?.groupTitle}
                      >
                        {iptv?.inf?.groupTitle}
                      </h2>
                    </div>
        ...
        <PaginationComponent
          mediaData={{ total_pages: iptvData.totalPages, ...iptvData }}
          page={page}
          setPage={setPage}
        />
      </div>
...
const ChannelsGridSkeleton = ({ basis }: { basis: string }) => {
  return (
    <div className='w-full h-fit pb-[100px] sm:pb-[30px]'>
      <div className='w-full h-full flex justify-start'>
        <div className='flex flex-wrap w-full justify-start items-center'>
          {Array(35)
            .fill(null)
            .map((_, i) => (
              <div
                key={i}
                style={{ flexBasis: basis }}
                className={`pl-5 relative min-w-0 shrink-0 grow-0 basis-1/2 sm:basis-1/4 lg:basis-1/6 2xl:basis-1/8 h-fit`}
              >
                <div className='group clickable'>
                  <span className='bg-yellow-500 w-[50px] h-[25px] text-white absolute top-2 left-7 z-[3] py-[2px] px-2 text-[0.8rem] rounded-3xl whitespace-nowrap flex items-center'></span>
