  ...
  return (
    <div className='w-full h-full max-h-[100dvh] overflow-scroll'>
      <div className='flex gap-y-10 flex-col w-full min-h-[100dvh] h-fit items-center justify-start p-6'>
        <div
          className={`font-bold text-3xl lg:text-[2.5rem] text-center ${
            iptvPlayerData.url === ''
              ? ' mt-[30dvh]'
              : 'iptv-player-app p-5 sm:p-10 min-w-[50vw] min-h-[50dvh] '
          }`}
        >
          Search TV channel to stream
        </div>
        {iptvPlayerData.url !== '' && (
          <IptvPlayer playerData={iptvPlayerData} />
        )}
        <form
          onSubmit={handleSubmit(onSubmit)}
          className='relative w-full max-w-[600px] flex h-fit items-center justify-center'
        >
          <Input
            {...register('search')}
            placeholder='Enter channel name'
            className='w-full rounded-3xl dark:bg-white dark:text-black'
          />
          <button
            type='submit'
            className='absolute right-0 bg-blue-500 hover:bg-blue-700 font-semibold py-0 px-2 rounded-r-full h-full'
          >
            <Search />
          </button>
        </form>
        ...
        <div className='flex flex-col gap-y-4 w-full h-full'>
          <IptvChannels
            query={query}
            setQuery={setQuery}
            setIptvPlayerData={setIptvPlayerData}
            searchBy={searchBy}
            group={group}
            page={page}
            setPage={setPage}
