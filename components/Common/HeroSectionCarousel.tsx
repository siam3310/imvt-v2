...
const HeroSectionCarouselSlide = ({
  ...
  return (
    <div
      className={`${theme === 'dark' ? 'embla__slide' : 'embla__slide-light'}`}
      key={index}
    >
      <Image
      ...
      <div className='flex items-center gap-3 absolute top-0 right-0 z-10 w-full h-[80%] p-[3rem]'>
        <div className='w-full flex flex-col justify-center gap-y-3 select-none'>
          <h3 className='font-bold text-[1.5rem] sm:text-[2rem] lg:text-[3rem]'>
          ...
          <div className='flex flex-wrap items-center gap-3'>
            ...
            {mediaData?.adult && (
              <span className='bg-red-500 p-2 text-[0.8rem] sm:text-[1rem] sm:py-2 sm:px-3 rounded-3xl'>
                NSFW
              </span>
            )}
            {mediaData?.genre_ids.map((genre, index) => (
              <span
                key={index}
                className='bg-green-500 p-2 text-[0.8rem] sm:text-[1rem] sm:py-2 sm:px-3 rounded-3xl'
              >
                {genre}
              </span>
            ))}
          ...
          <span className='text-lg max-h-[115px] text-[0.9rem] sm:text-[1rem] overflow-hidden'>
            {mediaData?.overview}...
          </span>
          <div className='flex justify-start items-center gap-x-2'>
            <Link
              href={`/${mediaData?.name ? 'tv' : 'movie'}/${
                mediaData?.id
              }/play`}
              className='bg-yellow-500 cursor-pointer w-24 sm:w-32 p-2 text-[0.8rem] sm:text-[1rem] sm:py-2 sm:px-3 rounded-xl text-center flex items-center gap-x-2'
            >
              <PlayCircle /> Play Now
            </Link>
            <Link
              href={`/${mediaData?.name ? 'tv' : 'movie'}/${mediaData?.id}`}
              className='bg-blue-500 cursor-pointer w-24 sm:w-32 p-2 text-[0.8rem] sm:text-[1rem] sm:py-2 sm:px-3 rounded-xl text-center'
            >
              More details
            </Link>
            <Popover>
              <PopoverTrigger asChild>
                <Button className='cursor-pointer w-24 sm:w-32 p-2 text-[0.8rem] sm:text-[1rem] sm:py-2 sm:px-3 rounded-xl text-center flex items-center gap-x-2'>
                  {watchlistType ? (
