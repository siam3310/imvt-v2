...
// import './Swiper.css'
const MediaSwiper = ({ data, loading, heading, upcoming }) => {
                                    ...
                                    <div className='hidden relative group-hover:block w-full h-full rounded-l-2xl'>
                                        ...
                                        <div className='absolute top-0 left-0'>
                                            <div className='flex flex-col justify-center gap-y-3 text-white p-[1rem]'>
                                                <h3 className='font-bold text-[1rem] max-h-[2.8rem] sm:max-h-[3rem] overflow-hidden'>{post.name || post.title || "unknown"}</h3>
                                                <div className='flex flex-wrap items-center gap-3 max-h-[1.8rem] overflow-hidden'>
                                                    {!upcoming && <span className='bg-yellow-500 py-[2px] px-2 text-[0.8rem] rounded-3xl whitespace-nowrap flex items-center'>
                                                        <Star fill="white" color='white' width={12} />&nbsp;{parseFloat(post.vote_average).toFixed(1)}
                                                    </span>}
                                                    {post.release_date && <span className='bg-orange-500 py-1 px-2 text-[0.8rem] rounded-3xl whitespace-nowrap flex items-center'>
                                        ...
                                        <div className='absolute bottom-2 w-full h-full px-2 text-white'>
                                            <div className='w-full h-full flex justify-center items-end gap-x-1 relative bottom-0 left-0'>
                                                ...
                                                <Link to={`/movie/${post.id}`} className='w-full rounded-full flex justify-center items-center bg-blue-500 py-1 clickable'><ExternalLink width={20} /></Link>
                                            </div>
                                        </div>
                                    </div>
                                </div>
